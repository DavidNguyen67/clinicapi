...
public class PhoneBloomFilter {
    ...
    public boolean mightExist(String phone) {
        ...
        log.info("Bloom filter indicates phone {} might exist", phone);
        return true;
    }

    private long[] getHashIndexes(String phone) {
        long[] indexes = new long[NUM_HASH_FUNCTIONS];
        byte[] bytes = phone.getBytes(StandardCharsets.UTF_8);
